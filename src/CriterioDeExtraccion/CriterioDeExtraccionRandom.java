...
public class CriterioDeExtraccionRandom extends CriterioDeExtraccion {

	private float alpha1;
	private float alpha2;

	public CriterioDeExtraccionRandom(float alpha1, float alpha2) {
		this.alpha1 = alpha1;
		this.alpha2 = alpha2;
	}
	...
	public List<TareaExtraccion> planearExtraccion(List<Pozo> listaPresionesDePozos, StockDePlantasSeparadoras plantasSeparadoras, Reservorio reservorio) {
		...
		for (Pozo pozo : listaPresionesDePozos) {
			...
			for (PlantaSeparadora planta: plantas) {
				...
				//random
				aExtraer = aExtraer * (new Random().nextFloat()) * (new Random().nextFloat());
