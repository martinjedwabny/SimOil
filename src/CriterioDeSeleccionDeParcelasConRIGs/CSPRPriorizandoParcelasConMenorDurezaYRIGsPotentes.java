...
public class CSPRPriorizandoParcelasConMenorDurezaYRIGsPotentes extends CriterioDeSeleccionDeParcelasConRIGs{

	@Override
	public List<AsignacionParcelaRIG> planearAsignaciones(List<Parcela> parcelas,
			ProveedoraDeRIGs proveedoraDeRIGs) {
		List<AsignacionParcelaRIG> asignaciones = new ArrayList();
		
		ordenarParcelasPorMenorDureza(parcelas);
		
		List<RIG> rigsNoAlquilados = proveedoraDeRIGs.RIGsDisponibles();
		ordenarRIGsPorMayorPotencia(rigsNoAlquilados);
		
		List<RIG> rigsAUsar = proveedoraDeRIGs.dameRIGsAlquilados();
		ordenarRIGsPorMayorPotencia(rigsAUsar);
